...
> [!NOTE]
> Koharu 会在你的本地设备上运行 ML 模型，确保数据隐私。不会向外部服务器发送任何数据。我们保证 Koharu 不会收集任何用户数据。
...
Koharu 通过 [candle](https://github.com/huggingface/candle) 支持 GGUF 格式的量化 LLM，并会基于系统语言环境预选模型。支持模型与推荐使用场景如下：
