...
#[allow(unused)]
fn get_qtensor_from_ggml(ct: &mut ggml_file::Content, names: &[String]) -> Result<QTensor> {
    for name in names {
        if let Ok(t) = ct.remove(name) {
            return Ok(t);
        }
    }
    bail!("cannot find tensor info for {}", names.join(" | "))
...
impl ModelWeights {
    ...
    pub fn from_ggml(mut ct: ggml_file::Content, gqa: usize) -> Result<Self> {
        ...
        for layer_idx in 0..ct.hparams.n_layer {
            ...
            let attention_q_norm = get_qtensor_from_ggml(
                &mut ct,
                &[
                    format!("{prefix}.attention.q_norm.weight"),
                    format!("{prefix}.attention_q_norm.weight"),
                ],
            )?;
            let attention_k_norm = get_qtensor_from_ggml(
                &mut ct,
                &[
                    format!("{prefix}.attention.k_norm.weight"),
                    format!("{prefix}.attention_k_norm.weight"),
                ],
            )?;
            ...
            layers.push(LayerWeights {
                ...
                attention_q_norm: RmsNorm::from_qtensor(attention_q_norm, 1e-5)?,
                attention_k_norm: RmsNorm::from_qtensor(attention_k_norm, 1e-5)?,
    ...
    pub fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        let n_expert = md_get("hunyuan-dense.expert_count")
            .and_then(|v| v.to_u32())
            .unwrap_or(0) as usize;
        let n_expert_used = md_get("hunyuan-dense.expert_used_count")
            .and_then(|v| v.to_u32())
            .unwrap_or(0) as usize;
        let head_count = md_get("hunyuan-dense.attention.head_count")
            .or_else(|_| md_get("llama.attention.head_count"))?
            .to_u32()? as usize;
        let head_count_kv = md_get("hunyuan-dense.attention.head_count_kv")
            .or_else(|_| md_get("llama.attention.head_count_kv"))?
            .to_u32()? as usize;
        let block_count = md_get("hunyuan-dense.block_count")
            .or_else(|_| md_get("llama.block_count"))?
            .to_u32()? as usize;
        let embedding_length = md_get("hunyuan-dense.embedding_length")
            .or_else(|_| md_get("llama.embedding_length"))?
            .to_u32()? as usize;
        let rope_dim = ct
            .metadata
            .get("hunyuan-dense.rope.dimension_count")
            .or_else(|| ct.metadata.get("llama.rope.dimension_count"))
            .and_then(|v| v.to_u32().ok())
            .map(|v| v as usize)
            .unwrap_or_else(|| embedding_length / head_count);
        // Strangely this value is generally 1e-6 in GGUF file but used to be 1e-5 by default.
        let rms_norm_eps = md_get("hunyuan-dense.attention.layer_norm_rms_epsilon")
            .or_else(|_| md_get("llama.attention.layer_norm_rms_epsilon"))?
            .to_f32()? as f64;

        let rope_freq_base = ct
            .metadata
            .get("hunyuan-dense.rope.freq_base")
            .or_else(|| ct.metadata.get("llama.rope.freq_base"))
            .and_then(|m| m.to_f32().ok())
            .unwrap_or(10000f32);
        let context_len = ct
            .metadata
            .get("hunyuan-dense.context_length")
            .or_else(|| ct.metadata.get("llama.context_length"))
            .or_else(|| ct.metadata.get("general.context_length"))
            .and_then(|v| v.to_u32().ok())
            .map(|v| v as usize)
            .unwrap_or(MAX_SEQ_LEN);
        let (cos, sin) = precomput_freqs_cis(rope_dim, rope_freq_base, context_len, device)?;
        ...
        for layer_idx in 0..block_count {
            ...
            let attention_q_norm = get_qtensor(
                &ct,
                reader,
                device,
                &[
                    ...
                    format!("{prefix}.attention.query_layernorm.weight"),
                ],
            )?;
            let attention_k_norm = get_qtensor(
                &ct,
                reader,
                device,
                &[
                    ...
                    format!("{prefix}.attention.key_layernorm.weight"),
                ],
            )?;
            ...
            layers.push(LayerWeights {
                ...
                attention_wv: QMatMul::from_qtensor(attention_wv)?,
                attention_wo: QMatMul::from_qtensor(attention_wo)?,
                attention_q_norm: RmsNorm::from_qtensor(attention_q_norm, rms_norm_eps)?,
                attention_k_norm: RmsNorm::from_qtensor(attention_k_norm, rms_norm_eps)?,
